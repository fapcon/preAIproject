...
func (x *NullString) UnmarshalJSON(data []byte) error {
	err := godecoder.NewDecoder().Decode(bytes.NewBuffer(data), &x.String)
	if err != nil {
		return err
	}
	x.Valid = true
...
func (x *NullBool) UnmarshalJSON(data []byte) error {
	err := godecoder.NewDecoder().Decode(bytes.NewBuffer(data), &x.Bool)
	if err != nil {
		return err
	}
	x.Valid = true
...
func (x *NullInt64) UnmarshalJSON(data []byte) error {
	err := godecoder.NewDecoder().Decode(bytes.NewBuffer(data), &x.Int64)
	if err != nil {
		return err
	}
	x.Valid = true
...
func (x *NullFloat64) UnmarshalJSON(data []byte) error {
	err := godecoder.NewDecoder().Decode(bytes.NewBuffer(data), &x.Float64)
	if err != nil {
		return err
	}
	x.Valid = true
