...
type Strategy struct {
	adapter storage.SQLAdapter
}

func NewStrategy(adapter storage.SQLAdapter) *Strategy {
	return &Strategy{adapter: adapter}
}

func (e *Strategy) Create(ctx context.Context, dto models.StrategyDTO) (int, error) {
	err := e.adapter.Create(ctx, &dto)
...
func (e *Strategy) GetByName(ctx context.Context, name string) (models.StrategyDTO, error) {
	var list []models.StrategyDTO
	err := e.adapter.List(ctx, &list, "strategy", utils.Condition{
		Equal: map[string]interface{}{"name": name},
	})
	if err != nil {
		return models.StrategyDTO{}, err
	}
	if len(list) < 1 {
		return models.StrategyDTO{}, fmt.Errorf("strategy storage: GetByName not found")
	}
	return list[0], err
}
