...
func NewFacebookAuth(conf config.OauthFacebook) *FacebookAuth {
	return &FacebookAuth{
		config: &oauth2.Config{
...
func (f FacebookAuth) Authenticate(ctx context.Context) (*social.User, error) {
	if f.token == nil {
		return &social.User{}, errors.New("facebook auth: token is required")
	}
